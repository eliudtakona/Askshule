...
// ⚠️ Basic password gate. For real protection, use Vercel's password
// protection feature or proper auth — this just keeps casual visitors out.
  ...
  return (
    <div style={{ background: '#F8FAFC', minHeight: '100vh' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg,#0A0F1E,#1E3A5F)', padding: '18px 20px' }}>
        ...
        <div style={{ color: '#64748B', fontSize: 11, marginTop: 2 }}>askshule.com/admin · All schools across the platform</div>
      </div>
      ...
      <div style={{ padding: 16 }}>
          ...
          <div key={i} onClick={() => navigate(`/school/${s.slug}`)}
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 11, color: '#64748B' }}>
                {s.location} · {s.studentCount} students · {s.tier}
              </div>
              <div style={{ fontSize: 10, color: '#94A3B8', marginTop: 2 }}>
                askshule.com/school/{s.slug}
              </div>
            </div>
            <span style={{
              background: s.status === 'Active' ? '#DCFCE7' : '#FEF3C7',
              color: s.status === 'Active' ? '#166534' : '#92400E',
              borderRadius: 8, padding: '3px 10px', fontSize: 10, fontWeight: 700, flexShrink: 0
            }}>{s.status}</span>
          </div>
        ))}
        ...
        <div style={{
          ...
          <div style={{ fontSize: 11, color: '#B45309', lineHeight: 1.6 }}>
            Open <strong>src/data/schools.js</strong> on GitHub, copy the TEMPLATE block,
            paste it below Grove Creek's entry, fill in the new school's details, and commit.
            The school is instantly live at <strong>askshule.com/school/their-slug</strong> —
